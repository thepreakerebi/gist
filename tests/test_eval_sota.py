...
import gist.eval.cli as eval_cli
from gist.eval.schemas import EvalVariant
from gist.eval.sota import run


def test_sota_cli_runs_benchmark_sweep_with_gateway(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        eval_cli,
        "SOTA_BENCHMARK_VARIANTS",
        [
            EvalVariant(name="gist_core", preset=CompressionPreset.BALANCED),
            EvalVariant(name="gist_aggressive", preset=CompressionPreset.AGGRESSIVE),
        ],
    )
    ...
    assert [variant["name"] for variant in payload["variants"]] == ["gist_core", "gist_aggressive"]
    assert payload["results"][0]["variants"][0]["answer_score"] == 1
    assert payload["results"][0]["baselines"][0]["answer_score"] == 1
    assert payload["results"][0]["baselines"][0]["selected_candidates"] > 0
    assert payload["results"][0]["baselines"][0]["selected"][0]["clip_path"]
...
def test_sota_cli_runs_single_config_with_gateway(tmp_path) -> None:
    dataset = tmp_path / "video_mme.jsonl"
    output_dir = tmp_path / "sota"
    video_root = tmp_path / "videos"
    video_root.mkdir()
