...
RESF = "/tmp/av7bfp16_results.jsonl"
...
model = Qwen2_5OmniForConditionalGeneration.from_pretrained(
    MID, torch_dtype=torch.float16, device_map="auto", enable_audio_output=False).eval()
...
def omni_answer(query, opts, imgs, auds):
    ...
    inputs = proc(text=text, audio=audio_in, images=image_in, return_tensors="pt", padding=True).to(next(model.parameters()).device)
    with torch.no_grad():
...
def prep_video(vid):
    ...
    video = f"/tmp/vids/videomme-{vid}.mp4"
    if cached:
...
print(f"RESULT7BFP16_FULL: {full_ok}/{n} ({full_ok/n:.0%}) frames=8 auds=4", flush=True)
print(f"RESULT7BFP16_GIST: {gist_ok}/{n} ({gist_ok/n:.0%}) avg_frames={gist_frames_tot/n:.1f} avg_auds={gist_auds_tot/n:.1f}", flush=True)
print("AV_BENCH_7BFP16_DONE", flush=True)
