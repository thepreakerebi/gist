...
def prep_video(vid):
    ...
    dur = float(r)
    fd = Path(f"/content/fr_{vid}"); fd.mkdir(exist_ok=True)
...
print(f"RESULT_FULL: {full_ok}/{n} ({full_ok/n:.0%}) frames=8 auds=4", flush=True)
print(f"RESULT_GIST: {gist_ok}/{n} ({gist_ok/n:.0%}) avg_frames={gist_frames_tot/n:.1f} avg_auds={gist_auds_tot/n:.1f}", flush=True)
print("AV_BENCH_DONE", flush=True)
